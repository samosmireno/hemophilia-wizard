...
| 16  | `rebalancing-agents`' three figure boxes: the caption instructs a click, no target is named, the exports draw PLACEHOLDER. What a box opens also decides its phone cost (items 39, 43). Designer.                                                                                                                                                                                                                                                                    | §11                   |
...
| 31  | `/explore`'s `<h1>` drawn 42, ships `text-4xl` (36) — the migration's largest fidelity loss; the heading sets three lines v the drawn four. If it matters, restore `text-[42px]` under §8's precedent (48 is further out). Confirmed 36px/three lines in a browser. Designer.                                                                                                                                                                                        | §17                   |
...
**`rebalancing-agents`.** No grid: prose, a centred row of three reserved boxes (224 × 192, squared
from drawn 227 × 185; group = the drawn 963), caption, then a `+` with the caption to its **left**.
**First chapter to transcribe its body rather than snap** (drawn ~24–26 → `text-2xl`; item 9
decided one way for one chapter); caption colour is item 15, agent colours item 14, the boxes open
nothing (item 16). The mechanisms click-through is **two cards in one dialog** (a
...
drawn 42, ships `text-4xl` 36 (item 31); class-label tracking is item 26. **The client cut the copy
on 2026-08-05**; the fitted sizes stand; the three-step ramp and `max-w-content` line-break cap are
now slack but kept. **The label row is a fixed height because the export says so four times** (all
